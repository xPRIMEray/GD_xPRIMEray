...
| object_island | 7/9 | ~49% (open oracle) | oracle diagnostics | ✓ Partial |
| corner_probe_reference | 6/9 | none | probe comparison only | ⚠ Placeholder |
| oracle_closure | 3/9 | none | internal only | 🔬 Experimental |
...
- **Traversal budget stress:** At 40×22 (smoke), 72.7% of rays exhaust the 700-step budget and resolve on the overrun step (step 701). At 160×112 (mini), traversal is within budget (avg 271 steps, max 273).
...
- "Sealed hit validation passed" sounds absolute but depends on the 701st-step overrun at smoke scale. The closure is valid; 72.7% of rays resolve on the extra step. This is traversal budget stress, not a closure failure. Budget stress is reported separately.
...
### Risk of misinterpretation
- **Hit rate ~1% looks like a test failure.** It is not. The sphere is a small deliberate target; 696 of 704 evaluated rays are classified as background (backdrop receiver) or miss. This is the expected distribution for this fixture geometry.
...
### Artifact coverage status
**Partial.** Two diagnostic panels are missing due to a pipeline gap in the field-view and signature generation for this study. The existing 7 panels are valid and correct. Recommend regenerating with updated `observatory_fixture_report.py` when field view artifacts are available.
...
### Purpose
A diagnostic oracle run targeting an "unresolved island" — a patch region in a 320×180 scene where transport is ambiguous or not converged. This is a ReferenceTransportOracle study, not a primary fixture. It tests the oracle's ability to surface convergence diagnostics without feeding those decisions back into the renderer.

### What it proves
- **Oracle diagnostic availability:** Oracle hit classification, transport ownership, and convergence data are accessible as post-process artifacts.
...
- **Oracle guardrail:** Oracle outputs are diagnostic only; they do not affect scheduling, hit selection, or renderer decisions.
...
### Acceptance gate
- Oracle diagnostic artifacts are present and consistent
- Hit rate and traversal data are non-trivial (neither 0% nor 100%)
- Oracle data does not appear in renderer execution path

### Risk of misinterpretation
- **"Unresolved island" sounds like the test broke.** It did not. An unresolved island is a deliberate diagnostic category — a region where the oracle flags convergence uncertainty. The 49% hit rate reflects genuine transport ambiguity in the target patch.
...
### Public-facing explanation
*Some regions of a scene are genuinely ambiguous — rays can reach multiple valid destinations, or the computation hasn't fully converged. This fixture uses a reference oracle to map those ambiguous regions and confirm the diagnostic system can locate and report them. The unresolved pixels are the result, not evidence of a bug.*
...
- If retained in the public index as `cathedral_probe`, it actively misleads about what xPRIMEray has been validated against.
...
### Purpose
ReferenceTransportOracle ROI sweep: a comparison study that fires the oracle at 94 sample points across a 320×180 scene at multiple step sizes, comparing oracle and production transport behavior. This is a passive diagnostic tool, not a fixture that proves closure or visual correctness.

### What it proves
- The oracle pipeline runs without crashing.
- Oracle data does not feed renderer decisions (guardrail confirmed).
- Passive comparison between oracle and production transport is available as a post-process artifact.

### What it does NOT prove
- Scene closure — no hit_diagnostics.csv, no miss_count, no closure contract.
- Visual correctness — beauty frame status unknown.
- Transport accuracy — the oracle comparison is a diagnostic, not a ground truth.
...
- "oracle_closure" sounds like it proves closure using an oracle. It proves the oracle runs. These are different claims.

### Public-facing explanation
Not yet ready for public explanation. This fixture is internal tooling — a research instrument for comparing oracle and production transport behavior. It is not a validation of scene closure or rendering quality.
