...
| **Query Observatory** | Observed | A storyboard artifact exists, but it has not yet been promoted into a stable catalog or canonical visitor-facing contract. |
...
For the full trust-vocabulary crosswalk, see the [Observatory Trust Model](./observatory_trust_model.md).

## Reading Rule

Use maturity as a trust label for Observatory evidence:

- **Low maturity (scores 0-1)** means useful, but easy to misread.
- **Middle maturity (scores 2-3)** means the artifact can support a local claim with caveats.
- **High maturity (scores 4-5)** means the artifact can teach the Observatory language itself.
