...
> This document is the authoritative reference for what each Observatory fixture is, what its acceptance gate tests, and what it does not claim. It is intended for engineers, researchers, and reviewers who need to interpret Observatory diagnostic outputs without misreading partial coverage as complete validation.
...
Use the [Observatory Trust Model](./observatory_trust_model.md) to distinguish observed artifacts from confirmed, characterized, or canonical ones. **Trust stage is evidence strength, not physical truth.**

---

## Fixture Overview

The table below shows panel inventory, transport results, and showcase readiness. For maturity stage assignments, see the Observatory Trust Model.
...
- Present: observation, scene geometry, domain ownership, closure map, traversal steps, combined overlay (derived), (budget stress partially covered)
...
### Expected diagnostics (current coverage: 7/9)
- Present: observation, scene geometry, domain ownership, closure map, traversal steps, combined overlay (derived), curvature signature
...
- This fixture does not confirm closure. Listing it next to hermetic_curved_room without clarification implies comparable validation depth.
...
- Present: observation, curvature field view, closure map (if available), traversal steps, budget stress, curvature signature
- **Missing: scene geometry, domain ownership, combined overlay (derived)**
...
### Purpose
ReferenceTransportOracle ROI sweep: a comparison study that runs reference integration at 94 sample points across a 320×180 scene at multiple step sizes, comparing reference and production transport behavior. This is a passive diagnostic tool, not a fixture that confirms closure or visual-render behavior.
...
- Present: observation, combined overlay (derived), curvature signature
- **Missing: scene geometry, curvature field view, domain ownership, closure map, traversal steps, budget stress** — 6 of 9 panels absent.
...
- "oracle_closure" sounds like it confirms closure using an oracle. It only shows the reference-integration pipeline ran for this study. These are different claims.
