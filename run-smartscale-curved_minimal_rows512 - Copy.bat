...
rem README: Runs CURVED_MINIMAL with SmartScale enabled (budget=rows_advanced, n=512), then parses render health output and prints the AutoCal summary block.

echo ============================================================
echo Running xPRIMEray SmartScale Render Test (CURVED_MINIMAL, ROWS512)
echo ============================================================
...
set "RUN_SUFFIX=smartscale_rows512"
set "SMARTSCALE_ARGS=--smartscale=1 --smartscale-goal=max_hits --smartscale-no-early-stop=1 --smartscale-budget=rows_advanced --smartscale-budget-n=512"
...
echo.
echo Finished SmartScale CURVED_MINIMAL ROWS512 test.
endlocal
goto :eof

:fail
echo.
echo FAILED SmartScale CURVED_MINIMAL ROWS512 test.
endlocal
exit /b 1
