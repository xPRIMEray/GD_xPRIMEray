...
echo ============================================
echo Running xPRIMEray Render Test (BLACKHOLE METRIC SEARCH COMPARE)
echo ============================================
...
set LOG_FILE=logs\ab_blackhole_metric_search_compare.log
set BUILD_LOG_FILE=logs\ab_blackhole_metric_search_compare.build.log
...
%GODOT_EXE% --path . -- --render-test --render-test-fixture=blackhole_minimal --render-test-profile=blackhole_metric_search_compare_fast --blackhole-transport-model=metric --lifecycle-stress=0 --smartscale=0 %* > %LOG_FILE% 2>&1

python tools\renderhealth_regress.py %LOG_FILE%

echo.
echo Finished BLACKHOLE METRIC search compare test.
