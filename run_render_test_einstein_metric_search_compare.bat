...
echo ============================================
echo Running xPRIMEray Render Test (EINSTEIN METRIC SEARCH COMPARE)
echo ============================================
...
set LOG_FILE=logs\ab_einstein_metric_search_compare.log
set BUILD_LOG_FILE=logs\ab_einstein_metric_search_compare.build.log
...
%GODOT_EXE% --path . -- --render-test --render-test-fixture=einstein_ring_minimal --render-test-profile=einstein_metric_search_compare_fast --einstein-transport-model=metric --lifecycle-stress=0 --smartscale=0 %* > %LOG_FILE% 2>&1

python tools\renderhealth_regress.py %LOG_FILE%

echo.
echo Finished EINSTEIN METRIC search compare test.
