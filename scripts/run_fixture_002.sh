...
# Fixture 002 is a non-destructive linear-row characterization harness:
# it reuses the hardened runtime/report pipeline from Fixture 001.
# Fixture 002 is not meant to be a visually dramatic departure from Fixture 001;
# it preserves the same radial field family while simplifying the source
# topology from a dense grid to a single centered row for cleaner attribution
# and DOE interpretation without adding geometry/field-position confounders.
...
resolve_ledger_python() {
  if [[ -n "${FIXTURE_002_LEDGER_PYTHON:-}" && -x "${FIXTURE_002_LEDGER_PYTHON}" ]]; then
    printf '%s\n' "${FIXTURE_002_LEDGER_PYTHON}"
    return 0
  fi
...
resolve_runtime_root() {
  if [[ -n "${FIXTURE_002_RUNTIME_ROOT:-}" ]]; then
    printf '%s\n' "${FIXTURE_002_RUNTIME_ROOT}"
    return 0
  fi
...
resolve_dotnet_exe() {
  if [[ -n "${FIXTURE_002_DOTNET_EXE:-}" ]]; then
    printf '%s\n' "${FIXTURE_002_DOTNET_EXE}"
    return 0
  fi
  ...
  echo "No Windows dotnet executable found for Fixture 002 runtime rebuild." >&2
  return 1
}
...
cleanup_stale_fixture_processes() {
  ...
  local scene_token="res://test-grin-basic-visual-linear-minimal.tscn"
...
emit_runtime_build_fingerprint() {
  ...
  export XPRIMERAY_BUILD_FINGERPRINT="fixture002_runtime_fingerprint_v1_git_${git_short}_utc_${dll_write_utc}_sha_${dll_sha}"
...
FIXTURE_ID="fixture_002"
SCENE_PATH="res://test-grin-basic-visual-linear-minimal.tscn"
LAUNCHER_TOKEN="run_fixture_002"
SETTLE_FRAMES="${FIXTURE_002_SETTLE_FRAMES:-12}"
MIN_RH_STEP="${FIXTURE_002_MIN_RH_STEP:-20}"
MIN_PROCESSED_ROWS="${FIXTURE_002_MIN_PROCESSED_ROWS:-64}"
CAPTURE_FILM_OPACITY="${FIXTURE_002_CAPTURE_FILM_OPACITY:-1.0}"
COMPARE_GRID="${FIXTURE_002_COMPARE_GRID:-1}"
COMPARE_CROSSHAIR="${FIXTURE_002_COMPARE_CROSSHAIR:-1}"
VISUAL_MODE="${FIXTURE_002_VISUAL_MODE:-diagnostic_flat}"
ANALYSIS_CAPTURE_MODE="${FIXTURE_002_ANALYSIS_CAPTURE_MODE:-categorical_final}"
SOURCE_HIGHLIGHT="${FIXTURE_002_SOURCE_HIGHLIGHT:-1}"
REQUESTED_TRANSPORT_MODEL="${FIXTURE_002_TRANSPORT_MODEL:-}"
REQUESTED_STEP_LENGTH="${FIXTURE_002_STEP_LENGTH:-}"
REQUESTED_MIN_STEP_LENGTH="${FIXTURE_002_MIN_STEP_LENGTH:-}"
REQUESTED_STEPS_PER_RAY="${FIXTURE_002_STEPS_PER_RAY:-}"
REQUESTED_TURN_THRESHOLD="${FIXTURE_002_TURN_THRESHOLD:-}"
REQUESTED_ERROR_TOLERANCE="${FIXTURE_002_ERROR_TOLERANCE:-}"
BASELINE_CAPTURE="${FIXTURE_002_BASELINE_CAPTURE:-}"
...
"$LEDGER_PYTHON_BIN" "$ROOT/tools/fixture_002_report.py" \
  ...
  --radial-bin-count "${FIXTURE_002_RADIAL_BIN_COUNT:-8}" \
  "${REPORT_ARGS[@]}"
