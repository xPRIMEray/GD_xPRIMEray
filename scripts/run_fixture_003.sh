...
# Fixture 003 is the first spatial-regime shift after Fixture 002:
# it preserves the same single centered source row and hardened reporting path
# while moving the field center moderately off-axis in +X to break radial
# symmetry without changing the broader fixture structure.
...
resolve_ledger_python() {
  if [[ -n "${FIXTURE_003_LEDGER_PYTHON:-}" && -x "${FIXTURE_003_LEDGER_PYTHON}" ]]; then
    printf '%s\n' "${FIXTURE_003_LEDGER_PYTHON}"
    return 0
  fi
...
resolve_runtime_root() {
  if [[ -n "${FIXTURE_003_RUNTIME_ROOT:-}" ]]; then
    printf '%s\n' "${FIXTURE_003_RUNTIME_ROOT}"
    return 0
  fi
...
resolve_dotnet_exe() {
  if [[ -n "${FIXTURE_003_DOTNET_EXE:-}" ]]; then
    printf '%s\n' "${FIXTURE_003_DOTNET_EXE}"
    return 0
  fi
  ...
  echo "No Windows dotnet executable found for Fixture 003 runtime rebuild." >&2
  return 1
}
...
cleanup_stale_fixture_processes() {
  ...
  local scene_token="res://test-grin-basic-visual-linear-offset-minimal.tscn"
...
emit_runtime_build_fingerprint() {
  ...
  export XPRIMERAY_BUILD_FINGERPRINT="fixture003_runtime_fingerprint_v1_git_${git_short}_utc_${dll_write_utc}_sha_${dll_sha}"
...
SCENE_PATH="res://test-grin-basic-visual-linear-offset-minimal.tscn"
LAUNCHER_TOKEN="run_fixture_003"
SETTLE_FRAMES="${FIXTURE_003_SETTLE_FRAMES:-12}"
MIN_RH_STEP="${FIXTURE_003_MIN_RH_STEP:-20}"
MIN_PROCESSED_ROWS="${FIXTURE_003_MIN_PROCESSED_ROWS:-64}"
CAPTURE_FILM_OPACITY="${FIXTURE_003_CAPTURE_FILM_OPACITY:-1.0}"
COMPARE_GRID="${FIXTURE_003_COMPARE_GRID:-1}"
COMPARE_CROSSHAIR="${FIXTURE_003_COMPARE_CROSSHAIR:-1}"
VISUAL_MODE="${FIXTURE_003_VISUAL_MODE:-diagnostic_flat}"
ANALYSIS_CAPTURE_MODE="${FIXTURE_003_ANALYSIS_CAPTURE_MODE:-categorical_final}"
SOURCE_HIGHLIGHT="${FIXTURE_003_SOURCE_HIGHLIGHT:-1}"
REQUESTED_TRANSPORT_MODEL="${FIXTURE_003_TRANSPORT_MODEL:-}"
REQUESTED_STEP_LENGTH="${FIXTURE_003_STEP_LENGTH:-0.040}"
REQUESTED_MIN_STEP_LENGTH="${FIXTURE_003_MIN_STEP_LENGTH:-}"
REQUESTED_STEPS_PER_RAY="${FIXTURE_003_STEPS_PER_RAY:-}"
REQUESTED_TURN_THRESHOLD="${FIXTURE_003_TURN_THRESHOLD:-2.4}"
REQUESTED_ERROR_TOLERANCE="${FIXTURE_003_ERROR_TOLERANCE:-0.010}"
BASELINE_CAPTURE="${FIXTURE_003_BASELINE_CAPTURE:-}"
...
"$LEDGER_PYTHON_BIN" "$ROOT/tools/fixture_003_report.py" \
  ...
  --radial-bin-count "${FIXTURE_003_RADIAL_BIN_COUNT:-8}" \
  "${REPORT_ARGS[@]}"
