...
public partial class EinsteinRingMinimalFingerprint : Node3D
{
	...
	private const string FixturePhotonBandArgPrefix = "--einstein-photon-band=";
	...
	public override void _Ready()
	{
		...
		GD.Print(
			$"[EinsteinFixture][PhotonBandContribution] mode={PhotonBandModeToToken(photonBandMode)} source={photonBandModeSource} enabled={(photonBandActive ? 1 : 0)} " +
		...
		GD.Print(
			$"[EinsteinFixture][ResolvedPhotonBand] source={bandResolveReason} mode={PhotonBandModeToToken(photonBandMode)} enabled={(effectiveBandResolved.enabled ? 1 : 0)} curve={effectiveBandResolved.curveType} modeFlags={effectiveBandResolved.modeFlags} " +
	...
	{
		GD.Print(
			$"[EinsteinFixture][PhotonBandMode] mode={PhotonBandModeToToken(mode)} source={source} authored_enabled={(authoredEnabled ? 1 : 0)} effective_enabled={(effectiveEnabled ? 1 : 0)}");
	}
